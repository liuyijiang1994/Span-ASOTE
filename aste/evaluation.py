...
class TagReader:
    ...
    def read_inst(cls, file, is_labeled, number, opinion_offset):
        ...
        # read AAAI2020 data
        for line in f:
            inputs = line['text'].split()  # sentence
            raw_entities = line['entity_list']  # entities
            raw_pairs = line['relation_list']  # triplets

            # re-format original triplets to jet_o tagging format
            new_raw_pairs = []
            for new_pair in raw_pairs:
                # change sentiment to value --> 0 neu, 1 pos, 2 neg
                if new_pair['predicate'] == "NEG":
                    polarity = 2
                elif new_pair['predicate'] == "POS":
                    polarity = 1
                else:
                    polarity = 0
                new_raw_pairs.append(
                    (
                        [new_pair['obj_tok_span'][0], new_pair['obj_tok_span'][1] - 1],
                        [new_pair['subj_tok_span'][0], new_pair['subj_tok_span'][1] - 1],
                        polarity
                    )
                )
            ...
            for raw_ent in raw_entities:
                if raw_ent['type'] == "SUBJECT":
                    ent_type = 'TARGET'
                else:
                    ent_type = 'OPINION'

                new_raw_entities.append(
                    (
                        [raw_ent['tok_span'][0], raw_ent['tok_span'][1] - 1], ent_type
                    )
                )

            new_raw_pairs.sort(key=lambda x: x[0][0])
            new_raw_entities.sort(key=lambda x: x[0][0])
            original_p += len(raw_pairs)

            # keep all original triplets during eval and test for calculating F1 score

            total_p += len(new_raw_pairs)
            output = (new_raw_entities, new_raw_pairs)

            # TODO: 检查output中，new raw pairs 为[]的情况，同时修正新增 new_raw_entities 的问题
