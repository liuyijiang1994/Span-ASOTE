...
class SpanModelDocument(BaseModel):
    ...
    def from_sentence(cls, x: Sentence):
        ...
        for t in x.entities:
            ner.append((t.start, t.end, t.ent_type))
...
class SpanModelPrediction(SpanModelDocument):
    ...
    def to_sentence(self) -> Sentence:
        ...
        entities = [
            Entity(start=start, end=end, ent_type=ent_type) for start, end, ent_type, value, prob in
            self.predicted_ner[0]
        ]
        return Sentence(
            id=int(self.doc_key),
...
def main(
    ...
    assert len(seeds) == len(paths), (seeds, paths)
